    ...
    private void process(Plugin plugin) {
        try {
            ...
            CacheManager pluginCacheManager =
                    new CacheManager(plugin.getLocalRepository().resolve(Settings.TEST_PLUGINS_DIRECTORY));
            plugin.collectMetadata(mavenInvoker);
            plugin.setMetadata(new PluginMetadata(pluginCacheManager, plugin).refresh());
